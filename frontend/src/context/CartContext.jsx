...
export const CartProvider = ({ children }) => {
    ...
    const addToCart = async (productId, quantity = 1, selectedSize= '', selectedColor= '') => {
        try{
            const res = await authFetch(`${BASEURL}/api/cart/add/`, {
                ...
                body: JSON.stringify({ product_id: productId, quantity: quantity, selected_size: selectedSize, selected_color: selectedColor })
            })
            if (!res.ok) {
    ...
    const updateQuantity = async (itemId, quantity) => {
        ...
        try{
            await authFetch(`${BASEURL}/api/cart/update-quantity/`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
